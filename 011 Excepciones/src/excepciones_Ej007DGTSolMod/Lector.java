...
public class Lector {
	...
	public static void addMuestra(Scanner sc, Conductor[] arrayConductores) {
		...
		if (Conductor.getNumCond() > 0) {
			mostrarConductoresSinMuestras(arrayConductores);
	...
	private static Muestra crearMuestra(Scanner sc) {
		...
		muestra.setFecha((LocalDate.now()).toString());
		muestra.setHora((LocalTime.now()).toString());
...
//		muestra.setHora(sc.nextLine());
	...
	//Mostrar los datos de los conductores sin muestras	
	public static void mostrarConductoresSinMuestras(Conductor[] arrayConductores) {
		
		if (Conductor.getNumCond() > 0) {
			System.out.println("Conductores: ");
			for (int i=0; i < Conductor.getNumCond(); i++) {
				System.out.println((i+1) +". " +arrayConductores[i].toStringSinMuestras());
			}
		} 
		else
			System.out.println("No hay conductores dados de alta.");

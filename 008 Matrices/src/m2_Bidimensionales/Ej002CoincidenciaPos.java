...
public class Ej002CoincidenciaPos {
	...
	public static void main(String[] args) {
		...
		System.out.println(coincidencia(numCoin, A));
		//Ej002Pos posicion = coincidencia(numCoin, A);
	}
	...
	public static Ej002Pos coincidencia(int num, int A[][]) {
		int i,j;
		boolean flag = false;
		Ej002Pos posicion = null;
		
		for(i=0; i<A.length && !flag; i++) {
			for(j=0; j<A[i].length && !flag; j++) {
				if (num == A[i][j]) {
					posicion = new Ej002Pos(i+1, j+1);
